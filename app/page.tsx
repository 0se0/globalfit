...
export default function Home() {
  ...
  async function handleAnalyzeCompany() {
    ...
    try {
      // 이미 공고가 파싱돼 있으면 그 직무 맞춤 로드맵을 받고, 아니면 회사 전반
      // 정보만 받는다 — 매칭 점수 계산(calculateMatch)에는 관여하지 않음
      const jobContext = parsedJob
        ? {
            requiredStacks: parsedJob.required_stacks.map((stack) => stack.raw),
            preferredStacks: parsedJob.preferred_stacks.map((stack) => stack.raw),
          }
        : undefined;
